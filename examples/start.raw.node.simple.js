...
http
  .createServer(async (nodeRequest, nodeResponse) => {
    try {
      const [_requestPathname, rawSearch] = (nodeRequest.url ?? '/').split('?');
      const searchParams = /** @type {Record<string, string | Array<string>>} */ (decodeQueryString(rawSearch));

      const responseHeaders = Object.create(null);

      responseHeaders['server'] = 'server name';
      responseHeaders['referrer-policy'] = 'same-origin';
      responseHeaders['x-content-type-options'] = 'nosniff';
      responseHeaders['x-frame-options'] = 'DENY';
      responseHeaders['x-xss-protection'] = '1;mode=block';
      responseHeaders['cache-control'] = 'max-age=180';
      responseHeaders['last-modified'] = 'Sun, 27 Oct 2024 10:17:29 GMT';
      responseHeaders['x-request-id'] = globalThis.crypto.randomUUID();
