...
describe('middleware / route', () => {
  ...
  it('match method GET with GET', async () => {
    const context = initTestContext({ requestUrl: 'http://localhost:8080/the-page' });
  ...
  it('match method HEAD with GET', async () => {
    const context = initTestContext({ requestMethod: 'HEAD', requestUrl: 'http://localhost:8080/the-page' });
    await chainAll([route('GET', '/the-page', setPathParamsOnResponseHeader)])(context);
    assert.equal(context.responseHeaders.get('x-path-params'), '{}');
  });

  it('match method POST with *', async () => {
    const context = initTestContext({ requestMethod: 'POST', requestUrl: 'http://localhost:8080/the-page' });
  ...
  it('do not match simple route', async () => {
    const context = initTestContext({ requestUrl: 'http://localhost:8080/another-page' });
    await chainAll([route('GET', '/the-page', setPathParamsOnResponseHeader)])(context);
    assert.equal(context.responseHeaders.get('x-path-params'), null);
  });

  it('match route with one param', async () => {
    const context = initTestContext({ requestUrl: 'http://localhost:8080/books/42' });
  ...
  it('match route with two params', async () => {
    const context = initTestContext({ requestUrl: 'http://localhost:8080/books/42/year/2024' });
  ...
  it('match route without optional param', async () => {
    const context = initTestContext({ requestUrl: 'http://localhost:8080/books' });
    await chainAll([route('GET', '/books{/:id}', setPathParamsOnResponseHeader)])(context);
    assert.equal(context.responseHeaders.get('x-path-params'), '{}');
  });

  it('match route with wildcare', async () => {
    const context = initTestContext({ requestUrl: 'http://localhost:8080/api/books/42' });
