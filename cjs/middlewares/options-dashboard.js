...
exports.optionsDashboard = optionsDashboard;

var _cacheControl = require("./cache-control.js");
...
function optionsDashboard(options = {}) {
  return async context => {
    ...
    if (context.requestUrl.search !== '') {
      ...
      const optionsString = (0, _options.serializeOptions)(options);
      return { ...context,
        responseStatus: 302,
        responseHeaders: {
          'location': [optionsString, '__dashboard__'].join('/')
        }
      };
    }
    ...
    function input(type, key, value, label) {
      const checked = (0, _options.optionsHasValue)(context.pathOptions, key, value) ? 'checked' : '';
      return `<label>
  ...
  <div class="blocks">
    ...
    <fieldset>
        ...
        <div class="choices">
            ...
            ${input('checkbox', 'cc', 'may', 'max-age=' + _cacheControl.ONE_YEAR)}
            ${input('checkbox', 'cc', 'smaz', 'stale-while-revalidate=0')}
            ${input('checkbox', 'cc', 'smay', 'stale-while-revalidate=' + _cacheControl.ONE_YEAR)}
            ${input('checkbox', 'cc', 'swry', 'stale-while-revalidate=' + _cacheControl.ONE_YEAR)}
            ${input('checkbox', 'cc', 'siey', 'stale-if-error=' + _cacheControl.ONE_YEAR)}
        </div>
    </fieldset>
